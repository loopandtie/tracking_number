...
class UPSTrackingNumberTest < Minitest::Test
  ...
  context "a UPS Mail Innovations International number" do
    ["801716432500182727", "801716432500182709"].each do |valid_number|
      should "return ups with valid number #{valid_number}" do
        should_be_valid_number(valid_number, TrackingNumber::UPSMailInnovationsInternational, :ups)
      end
      ...
      should "detect #{valid_number} regardless of spacing" do
        should_detect_number_variants(valid_number, TrackingNumber::UPSMailInnovationsInternational)
      end
    end
  end
